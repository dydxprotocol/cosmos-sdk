...
// NEXT: right type?
func (s *Store) Query(query *types.RequestQuery) (*types.ResponseQuery, error) {
	//TODO implement me
	panic("implement me")
}
...
// NEXT: type
func (s *Store) CacheWrapWithListeners(storeKey types.StoreKey, listener *types.MemoryListener) types.CacheWrap {
...
// NEXT: not sure about this.
func (s *Store) WorkingHash() []byte {
	fmt.Printf("IAVLV2 Get WorkingHash: %x\n", s.Tree.Hash())
	return s.Tree.Hash()
}
