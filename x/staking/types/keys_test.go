...
func TestProposerKey(t *testing.T) {
	tests := []struct {
		name    string
		...
		{
			name:    "validator 3",
			valAddr: sdk.ValAddress(keysAddr3),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// GetProposerKey should create correct key that follows the format of:
			// prefix + length byte + address bytes
			key := types.GetProposerKey(tt.valAddr)
			require.True(t, bytes.HasPrefix(key, types.ProposerKeyPrefix))

			prefixLen := len(types.ProposerKeyPrefix)
			lengthByte := key[prefixLen]
			expectedLen := len(tt.valAddr.Bytes())
			require.Equal(t, byte(expectedLen), lengthByte, "length byte should match address length")

			addrBytes := key[prefixLen+1:]
			require.Equal(t, tt.valAddr.Bytes(), addrBytes, "address bytes should match")

			// GetValOpAddrFromProposerKey should extract the correct address
			extractedAddr := types.GetValOpAddrFromProposerKey(key)
			require.Equal(t, tt.valAddr.Bytes(), extractedAddr)
		})
	}
}
